                ...
                case "buy_seeds":
                    {
                        ...
                        else if (PO.Type == "grange")
                        {
                            if (!Room.GrangeEnabled)
                            {
                                Client.SendWhisper("Debes ir a la Granja para comprar ese producto.", 1);
                                return;
                            }
                        }
                        #endregion
                        ...
                        else if(Action == "buy_grua")
                        {
                            ...
                            Client.GetPlay().CarEffectId = vehicle.EffectID;//Guarda el enable del último auto en conducción.
                                                                            //Client.GetRoomUser().ApplyEffect(vehicle.EffectID); - No Pone efecto con DrivingIncar = True
                                                                            //Client.GetRoomUser().FastWalking = true; - No FastWalking sin efecto
                            #endregion
                        ...
                        else if(Action == "buy_seeds")
                        {
                            #region Buy Seeds
                            #region Conditions
                            if (Client.GetPlay().FarmSeeds)
                            {
                                Socket.Send("compose_products|productmsg|Ya tienes un puñado se semillas en tu inventario. ((Ahora ve sobre un surco dale doble clic.))");
                                return;
                            }
                            #endregion
                            ...
                            if (BTile == null)
                            {
                                BTile = Room.GetRoomItemHandler().GetFloor.FirstOrDefault(x => x.GetBaseItem().ItemName.ToLower() == "actionpoint02" && x.Coordinate == Client.GetRoomUser().Coordinate);
                                if (BTile != null)
                                    WeedSeeds = true;
                            }
                            if (BTile == null)
                            {
                                Socket.Send("compose_products|productmsg|Debes acercarte al punto de venta de semillas.");
                                return;
                            }
                            #endregion
                            ...
                            if (!WeedSeeds)
                            {
                                int Price = PO.Price;//RoleplayManager.SeedsPrice;
                                ...
                                RoleplayManager.Shout(Client, "*Compra un puñado de semillas y paga $" + Price + "*", 5);
                                Socket.Send("compose_products|productmsg_green|Has comprado unas semillas y pagaste $" + Price + ". ((Ahora ve sobre un surco y dale doble clic))");
                                Client.GetHabbo().Credits -= Price;
                                Client.GetHabbo().UpdateCreditsBalance();
                                Client.GetPlay().FarmSeeds = true;
                                Client.GetPlay().CooldownManager.CreateCooldown("comprar", 1000, 3);
                            }
                            #endregion
                ...
                case "open_grange":
                    {
                        ...
                        if (!Room.GrangeEnabled)
                        {
                            Client.SendWhisper("Debes ir a una granja para comprar semillas.", 1);
                            return;
                        }
                        #endregion
                        ...
                        if (BTile == null)
                        {
                            Client.SendWhisper("Debes acercarte al punto de venta de semillas.", 1);
                            return;
                        }
                        #endregion
                        ...
                        {
                            string html = "";
                            List<Product> PO = ProductsManager.getProductsByType("grange");
                            if (PO != null && PO.Count > 0)
                            {
                                foreach (var product in PO)
                                {
                                    html += "<div class= \"product\">";
                                    html += "<div class=\"product-box\" style=\"background-image: url('" + RoleplayManager.CdnURL + "/ws_overlays/Products/resources/images/" + product.ProductName + ".png');background-position: center center;background-repeat: no-repeat;\">";
                                    html += "<center><b>" + product.DisplayName + "</b></center>";
                                    html += "</div>";
                                    html += "<div class=\"product-footer\">";
                                    html += "<div id=\"Prod_Price\">$ " + String.Format("{0:N0}", product.Price) + "</div>";
