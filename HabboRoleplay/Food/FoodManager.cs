...
namespace Plus.HabboRoleplay.Food
{
    class FoodManager
    {
        ...
        public static void Initialize()
        {
            ...
            {
                DB.SetQuery("SELECT * from `play_food`");
                Foods = DB.getTable();
                ...
                if (Foods != null)
                {
                    foreach (DataRow Food in Foods.Rows)
                    {
                        ...
                        Food newFood = new Food(Name, Type, ItemId, ExtraData, Cost, Health, Hunger, ServeText, EatText, Servable);
                        FoodList.TryAdd(Name, newFood);
                    }
                }
            }

            log.Info("Loaded " + FoodList.Count + " roleplay food.");
        }
        ...
        /// </summary>
        public static string GetServableItems(GameClient Session)
        {
            ...
            {
                if (!PlusEnvironment.GetGame().GetGroupManager().HasJobCommand(Session, item.Type.ToLower()))
                  continue;
        ...
        public static string GetServableItemsName(GameClient Session)
        {
            if (!PlusEnvironment.GetGame().GetGroupManager().HasJobCommand(Session, "serve") && !Session.GetHabbo().GetPermissions().HasRight("corporation_rights"))
                return "";
            ...
            foreach (var item in FoodList.Values)
            {
                if (!PlusEnvironment.GetGame().GetGroupManager().HasJobCommand(Session, item.Type.ToLower()) && !Session.GetHabbo().GetPermissions().HasRight("corporation_rights"))
                    continue;
                if (!List.Contains(item))
                    List.Add(item);
            }
        ...
        public static string GetOrdenableItemsName()
        {
            StringBuilder Items = new StringBuilder();
            var List = new List<Food>();
