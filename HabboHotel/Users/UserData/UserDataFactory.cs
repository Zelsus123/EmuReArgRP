...
namespace Plus.HabboHotel.Users.UserDataManagement
{
    public class UserDataFactory
    {
        public static UserData GetUserData(string SessionTicket, out byte errorCode)
        {
            ...
            DataRow SubscriptionsInfo = null;

            using (IQueryAdapter dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                dbClient.SetQuery("SELECT `id`,`username`,`rank`,`motto`,`look`,`gender`,`last_online`,`ip_last`,`credits`,`activity_points`,`home_room`,`block_newfriends`,`hide_online`,`hide_inroom`,`account_created`,`vip_points`,`machine_id`,`volume`,`chat_preference`,`focus_preference`, `pets_muted`,`bots_muted`,`advertising_report_blocked`,`last_change`,`gotw_points`,`ignore_invites`,`time_muted`,`allow_gifts`,`friend_bar_state`,`disable_forced_effects`,`allow_mimic`,`rank_vip`,`helper_level` FROM `users` WHERE `rdpticket` = @sso LIMIT 1");
                ...
                // Jeihden && Tester
                if (UserId != 18 && UserId != 4110)
                    PlusEnvironment.GetGame().GetClientManager().LogClonesByIpOut(Convert.ToString(dUserInfo["ip_last"]));
                ...
                dbClient.SetQuery("SELECT * FROM `user_subscriptions` WHERE `user_id` = '" + UserId + "' ORDER BY id DESC LIMIT 1");
                SubscriptionsInfo = dbClient.getRow();

                dbClient.RunQuery("UPDATE `users` SET `online` = '1', `rdpticket` = '' WHERE `id` = '" + UserId + "' LIMIT 1");
            }
            ...
            */
            ...
            foreach (DataRow dRow in dRooms.Rows)
            {
                using (IQueryAdapter dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
                {
                    dbClient.SetQuery("SELECT * FROM `play_rooms` WHERE `id` = " + Convert.ToInt32(dRow["id"]) + " LIMIT 1");
            ...
            {
                VIPExpire = Convert.ToInt32(SubscriptionsInfo["expire_date"]);
            }

            Habbo user = HabboFactory.GenerateHabbo(dUserInfo, UserInfo, VIPExpire);
