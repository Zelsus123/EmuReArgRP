...
namespace Plus.HabboHotel.Users.Authenticator
{
    ...
    {
        public static Habbo GenerateHabbo(DataRow Row, DataRow UserInfo, int VIPExpire)
        {
            return new Habbo(Convert.ToInt32(Row["id"]), Convert.ToString(Row["username"]), Convert.ToInt32(Row["rank"]), Convert.ToString(Row["motto"]), Convert.ToString(Row["look"]),
                ...
                PlusEnvironment.EnumToBool(Row["allow_mimic"].ToString()), Convert.ToInt32(Row["rank_vip"]), Convert.ToInt32(Row["helper_level"]), VIPExpire, Convert.ToString(Row["ip_last"]));
        }
    }
}