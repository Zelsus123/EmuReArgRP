...
namespace Plus.HabboHotel.Rooms
{
    public class Gamemap
    {
        ...
        public bool CanRollItemHere(int x, int y, GameClient Session)
        {
            if (Session == null)
                return false;
        ...
        public bool GetMaxZItemForSquare(Point Square, out Item Item)
        {
            List<Item> Items = GetAllRoomItemForSquare(Square.X, Square.Y);
            Item = null;
            double HighestZ = -1;
        ...
        public bool IsValidStep2(RoomUser User, Vector2D From, Vector2D To, bool EndOfPath, bool Override)
        {
            ...
            if (Override && (User.GetClient() != null && !User.GetClient().GetPlay().DrivingCar && !User.GetClient().GetPlay().Pasajero))
                return true;

            if ((User.IsBot && User.BotData.Name.Contains("Taxi#")) || (!User.IsBot && User.GetClient().GetHabbo().TaxiChofer > 0))
                return true;
            /*
             * 0 = blocked
             * 1 = open
            ...
            if (Items.Count > 0)
            {
                ...
                if (HasBed)
                {
                    if ((User.IsBot && User.BotData.AiType == BotAIType.TAXIBOT) || (User.GetClient() != null && (User.GetClient().GetPlay().DrivingCar || User.GetClient().GetPlay().Pasajero || User.GetClient().GetHabbo().EscortID > 0 || User.GetClient().GetHabbo().Escorting > 0 || User.GetClient().GetHabbo().TaxiChofer > 0 || User.GetClient().GetHabbo().TaxiPassenger > 0)))
                    {
                        //User.GetClient().SendWhisper("No pasa");
                        return false;
                    }
                }

                bool IsArrow = Items.ToList().Where(x => x != null && x.GetBaseItem().InteractionType == InteractionType.ARROW).Count() > 0;

                if (IsArrow)
                {
                    if (User.GetClient() != null && User.GetClient().GetHabbo().Escorting > 0)
                        return false;
                }
            }
        ...
        public bool IsValidStep(Vector2D From, Vector2D To, bool EndOfPath, bool Override, RoomUser User, bool Roller = false)
        {
            if (!ValidTile(To.X, To.Y))
                return false;
            ...
            if (_room.RoomBlockingEnabled == 0 && SquareHasUsers(To.X, To.Y) && (User != null && User.GetClient() != null && !User.GetClient().GetPlay().Chofer && !User.GetClient().GetPlay().EscortingWalk))
                return false;
