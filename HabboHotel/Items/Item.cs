...
using Plus.HabboHotel.Items.Interactor.Roleplay;
...
namespace Plus.HabboHotel.Items
{
    ...
    public class Item
    {
        ...
        public Item(int Id, int RoomId, int BaseItem, string ExtraData, int X, int Y, Double Z, int Rot, int Userid, int Group, int limitedNumber, int limitedStack, string wallCoord, Room Room = null, RentableSpaceData House = null)
        {
            ItemData Data = null;
        ...
        public IFurniInteractor Interactor
        {
            get
            {
                ...
                switch (GetBaseItem().InteractionType)
                {
                    ...
                    case InteractionType.WATER_PUMP:
                        return new InteractorWaterPump();
                    case InteractionType.GUILD_ITEM:
                        return new InteractorGroupFlag();
                    case InteractionType.WARDROBE:
                        return new InteractorWardrobe();
                    case InteractionType.SLOTS:
                        return new InteractorSlots();
                    case InteractionType.NEST_DIRT:
                        return new InteractorNestDirst();
                    case InteractionType.WEED_PLANTPOT:
                        return new InteractorWeedPlantPoint();
                        ...
                        case InteractionType.DICE:
                            {
                                ...
                                UpdateState();
                                //left off here
                                Habbo theUserrollingDice = PlusEnvironment.GetHabboById(InteractingUser);
                                if (theUserrollingDice.casinoEnabled)
                                    theUserrollingDice.casinoEvent(ExtraData);
        ...
        public void UserWalksOnFurni(RoomUser user)
        {
            ...
            if (GetBaseItem().InteractionType == InteractionType.COMODIN)
            {
                ...
                if(C != null)
                {
                    ...
                    PlusEnvironment.GetGame().GetWebEventManager().ExecuteWebEvent(user.GetClient(), Data[0], Data[1]);
                    
                    #region Tutorial Step Check
                    if (Session.GetPlay().TutorialStep == 18 && GetRoom().PhoneStoreEnabled && GetRoom().Type.Equals("public"))
                    {
                        PlusEnvironment.GetGame().GetWebEventManager().SendDataDirect(Session, "compose_tutorial|19");
                    }
                    else if (Session.GetPlay().TutorialStep == 23 && GetRoom().BuyCarEnabled && GetRoom().Type.Equals("public"))
                    {
                        PlusEnvironment.GetGame().GetWebEventManager().SendDataDirect(Session, "compose_tutorial|25");
                    }
                    else if (Session.GetPlay().TutorialStep == 27 && GetRoom().MallEnabled && GetRoom().Type.Equals("public"))
                    {
                        PlusEnvironment.GetGame().GetWebEventManager().SendDataDirect(Session, "compose_tutorial|29");
                    }
                    #endregion
            ...
            // Check if is Taxi Stop
            if(GetBaseItem().ItemName == "bump_signs" && (user.Path.Count - user.PathStep) <= 0)
            {
                PlusEnvironment.GetGame().GetWebEventManager().ExecuteWebEvent(Session, "event_taxi", "open");
            }
        }
