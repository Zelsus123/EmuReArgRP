                                ...
                                */
                                ...
                                if (GetRooms != null)
                                {
                                    foreach (DataRow Row in GetRooms.Rows)
                                    {
                                        using (IQueryAdapter dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
                                        {
                                            dbClient.SetQuery("SELECT * FROM `play_rooms` WHERE `id` = " + Convert.ToInt32(Row["id"]) + " LIMIT 1");
                                            DataRow RPRow = dbClient.getRow();

                                            RoomData RoomData = PlusEnvironment.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(Row["id"]), Row, RPRow);
                                            if (RoomData != null && !Results.Contains(RoomData))
                                                Results.Add(RoomData);
                                        }
                                    }
                                ...
                                if (Table != null)
                                {
                                    foreach (DataRow Row in Table.Rows)
                                    {
                                        ...
                                        using (IQueryAdapter dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
                                        {
                                            ...
                                            RoomData RData = PlusEnvironment.GetGame().GetRoomManager().FetchRoomData(Convert.ToInt32(Row["id"]), Row, RPRow);
                                            if (RData != null && !Results.Contains(RData))
                                                Results.Add(RData);
                                        }
                                    }
