    ...
    class DeleteGroupEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            using (IQueryAdapter dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                dbClient.RunQuery("DELETE FROM `play_jobs_ranks` WHERE `job` = '" + Group.Id + "'");
            }

            //Unload it last.
            PlusEnvironment.GetGame().GetRoomManager().UnloadRoom(Room, true);

            string type = "Empresa eliminada";
            if (Group.GType == 2)
                type = "Trabajo eliminado";
            if (Group.GType == 3)
                type = "Banda eliminida";

                //Say hey!
                Session.SendNotification(type + " satisfactoriamente.");
            return;
        }
    }
}
