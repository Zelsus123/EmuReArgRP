            ...
            using (IQueryAdapter dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
            {
                ...
                if (Visits != null)
                {
                    ...
                    foreach (DataRow Visit in Visits.Rows)
                    {
                        ...
                        Room Room = PlusEnvironment.GetGame().GetRoomManager().LoadRoom(Convert.ToInt32(Visit["room_id"]));
                        ...
                        dbClient.SetQuery("SELECT user_id,DATE_FORMAT(from_unixtime(timestamp),'%k:%i'),message FROM `chatlogs` WHERE room_id = " + Convert.ToInt32(Visit["room_id"]) + "  ORDER BY id DESC LIMIT 150");
                        ...
                        if (Chatlogs != null)
                        {
                            ...
                            {
                                UserCache Habbo = PlusEnvironment.GetGame().GetCacheManager().GenerateUser(Convert.ToInt32(Log["user_id"]));

                                if (Habbo == null)
                                    continue;
                                //base.WriteInteger(((int)PlusEnvironment.GetUnixTimestamp() - Convert.ToInt32(Log["timestamp"])) * 1000);
                                //int Time = ((int)PlusEnvironment.GetUnixTimestamp() - Convert.ToInt32(Log["timestamp"]) * 1000);
                                //int Time = (int)PlusEnvironment.GetUnixTimestamp();
                                //DateTime time = new DateTime(Convert.ToInt32(Time));
                                base.WriteString(Convert.ToString(Log[1]));
