...
namespace Plus.HabboHotel.Rooms
{
    public class RoomItemHandling
    {
                ...
                if (Item.IsFloorItem)
                {
                    if (!_room.GetGameMap().ValidTile(Item.GetX, Item.GetY))
                    {
                        ...
                        if (Client != null)
                        {
                            ...
                            Client.GetHabbo().GetInventoryComponent().UpdateItems(FromDB);
                        }
                        continue;
                    }
        ...
        public void RemoveFurniture(GameClient Session, int pId, bool WasPicked = true)
        {
            ...
            if (Item.GetBaseItem().ItemName.ToLower() == "olympics_c16_treadmill" || Item.GetBaseItem().ItemName.ToLower() == "olympics_c16_crosstrainer" || Item.GetBaseItem().InteractionType == InteractionType.TRASH_CAN || Item.GetBaseItem().InteractionType == InteractionType.SHOWER)
            {
                Item.UpdateCounter = 0;
                Item.UpdateNeeded = false;
            }
                ...
                foreach (Item Roller in _rollers.Values.ToList())
                {
                    ...
                    if (ItemsOnRoller.Count > 0)
                    {
                        foreach (Item rItem in ItemsOnRoller.ToList())
                        {
                            ...
                            GameClient Session = PlusEnvironment.GetGame().GetClientManager().GetClientByUserID(rItem.UserID);
                ...
                if (_movedItems.Count > 0)
                {
                    foreach (Item Item in _movedItems.Values.ToList())
                    {
                        using (IQueryAdapter dbClient = PlusEnvironment.GetDatabaseManager().GetQueryReactor())
                        {
                            ...
                            {
                                if (Item.GetBaseItem().InteractionType == InteractionType.SHOWER || Item.GetBaseItem().InteractionType == InteractionType.GUILD_GATE)
                                    Item.ExtraData = "0";
        ...
        public bool SetFloorItem(GameClient Session, Item Item, int newX, int newY, int newRot, bool newItem, bool OnRoller, bool sendMessage, bool updateRoomUserStatuses = false)
        {
            ...
            if (!_room.GetGameMap().ValidTile(newX, newY) || (_room.GetGameMap().SquareHasUsers(newX, newY) && !Item.GetBaseItem().IsSeat && !Session.GetPlay().IsFarming))
            {
                if (NeedsReAdd)
            ...
            {
                if (!_room.GetGameMap().ValidTile(Tile.X, Tile.Y) ||
                    (_room.GetGameMap().SquareHasUsers(Tile.X, Tile.Y) && !Item.GetBaseItem().IsSeat && Session != null && !Session.GetPlay().IsFarming) ||
                    (Session != null && !Session.GetPlay().isParking && !_room.CheckTerrain(Session, Tile.X, Tile.Y) && _room.OwnerId != Session.GetHabbo().Id && !_room.CheckRights(Session, false, true)))// New Poner items en área Terreno
                {
                    if (NeedsReAdd)
                    {
                        _room.GetGameMap().AddToMap(Item);
            ...
            if (!OnRoller)
            {
                ...
                if (_room.GetGameMap().Model.SqState[newX, newY] != SquareState.OPEN && !Item.GetBaseItem().IsSeat && !Session.GetPlay().IsFarming)
                {
                    return false;
                }
                ...
                foreach (ThreeDCoord Tile in AffectedTiles.Values)
                {
                    if (_room.GetGameMap().Model.SqState[Tile.X, Tile.Y] != SquareState.OPEN &&
                        !Item.GetBaseItem().IsSeat && (Session != null && !Session.GetPlay().IsFarming))
                    {
                        if (NeedsReAdd)
                        {
                            //AddItem(Item);
                ...
                if (!Item.GetBaseItem().IsSeat && !Item.IsRoller && (Session != null && !Session.GetPlay().IsFarming))
                {
                    foreach (ThreeDCoord Tile in AffectedTiles.Values)
                    {
            ...
            if (newItem)
            {
                if (_floorItems.ContainsKey(Item.Id)/* && !Session.GetPlay().DrivingCar && !Session.GetPlay().IsFarming*/)// Posible desaparición de auto
                {
                    if (Session != null)
        ...
        public bool SetFloorItemRP(GameClient Session, Item Item, int newX, int newY, int newRot, bool newItem, bool OnRoller, bool sendMessage, bool updateRoomUserStatuses = false, bool ToDB = true, Item SpaceItem = null, double newZ = 1000.0, bool PlacedByRoleplay = false)
        {
            double origZ = newZ;
