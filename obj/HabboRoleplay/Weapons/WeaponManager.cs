...
namespace Plus.HabboRoleplay.Weapons
{
    public static class WeaponManager
    {
        ...
        public static void Initialize()
        {
            ...
            {
                DB.SetQuery("SELECT * FROM `play_weapons`");
                DataTable WeaponTable = DB.getTable();

                if (WeaponTable == null)
                    log.Error("Failed to load Roleplay Weapons!");
                else
        ...
        private static void ProcessWeaponsTable(DataTable WeaponTable)
        {
            foreach (DataRow Row in WeaponTable.Rows)
            {
                ...
                Weapon Weapon = new Weapon(ID, WeaponUnfriendlyName, WeaponName, FiringText, EquipText, UnEquipText, ReloadText, Energy, EffectID, HandItem, Range, MinDamage, MaxDamage, ClipSize, ReloadTime, Cost, CostFine, Stock, LevelRequirement, true, ClipSize, WLife, 0);
            ...
            log.Info("Loaded " + Weapons.Count + " roleplay weapons.");
        }
