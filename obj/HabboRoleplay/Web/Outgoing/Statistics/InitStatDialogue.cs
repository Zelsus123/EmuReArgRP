...
namespace Plus.HabboHotel.Roleplay.Web.Incoming.General
{
    ...
    /// </summary>
    class InitStatDialogue : IWebEvent
    {
        /// <summary>
        ...
        public void Execute(GameClient Client, string Data, IWebSocketConnection Socket)
        {
            ...
            if (Client != null)
            {
                ...
                Socket.Send("compose_initcharacter|" + CachedDataString);
            }
            else
            {
                using (UserCache CachedClient = PlusEnvironment.GetGame().GetCacheManager().GenerateUser(Convert.ToInt32(Socket.ConnectionInfo.Path.Trim().Split('/')[1])))
                {
                    ...
                    Socket.Send("compose_initcharacter|" + GetUserComponent.ReturnUserStatistics(CachedClient));
                }
            }
        }
    }
