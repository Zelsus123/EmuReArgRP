...
namespace Plus.Communication.Packets.Incoming.Rooms.Furni.RentableSpaces
{
    class PurchaseRentableSpaceEvent : IPacketEvent
    {
        public void Parse(GameClient Session, ClientPacket Packet)
        {
            ...
            if (Item != null)
            {
                if (Item.GetBaseItem().Id == 111111118)
                ...
                {
                    Console.WriteLine("==> " + Item.GetBaseItem().Id);
        ...
        {
            Console.WriteLine("Entramos al ProcessHouse Purchase => Item: "+ Item);
            var House = PlusEnvironment.GetGame().GetHouseManager().GetHouseBySignItem(Item);
            Console.WriteLine("GetHouseBysingItem => House: "+ House);
            if (House != null)
            {
                Console.WriteLine("House no nula");
                if (CanPurchase(Session, House))
                {
                    ...
                    if (House.OwnerId != Session.GetHabbo().Id)
                    {
                        ...
                        RoleplayManager.Shout(Session, "*Purchases the house for $" + Cost + "*", 4);
                    }
                    else
                    {
                        House.BuyHouse(Session, 0);
                        RoleplayManager.Shout(Session, "*Takes their house off the market*", 4);
                    }
        ...
        public void ProcessPurchase(GameClient Session, Item Item)
        {
            ...
            RoleplayManager.Shout(Session, "*Purchases the plot of land for $" + Cost + "*", 4);
            Session.SendMessage(new RentableSpaceComposer(Item, Session));
        }
