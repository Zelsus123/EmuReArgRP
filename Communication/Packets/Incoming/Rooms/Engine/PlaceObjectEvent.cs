...
namespace Plus.Communication.Packets.Incoming.Rooms.Engine
{
    class PlaceObjectEvent : IPacketEvent
    {
        public void Parse(HabboHotel.GameClients.GameClient Session, ClientPacket Packet)
        {
            ...
            string RawData = Packet.PopString().Trim(); // Fix last space blank on nitro by Jeihden
            Data = RawData.Split(' ');
            ...
            if (Room.CheckTerrain(Session, Data))
                MyTerrain = true;


            if (!HasRights && !MyTerrain)
            {
                ...
                case InteractionType.JUKEBOX:
                    {
                        ...
                        {
                            Session.SendNotification("Sólo es posible colocar 1 Jukebox por Zona.");
                            return;
                        }
                        break;
                    }
