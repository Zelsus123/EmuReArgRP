...
namespace Plus.Communication.Packets.Incoming.Navigator
{
    class NewNavigatorSearchEvent : IPacketEvent
    {
        ...
        {
            if (!Session.GetHabbo().GetPermissions().HasRight("can_open_navigator"))
            {
                Session.SendNotification("No tienes permitido usar el navegador. ¡Transportate como lo harías en la vida real!");
                return;
            }
            else
            {
